...
How I resolved it:
Solution to Find #1:
We replaced raw query construction with a prepared statement using parameterized queries. This ensures that user input is treated as data rather than executable SQL code. The query now uses placeholders (?) for the search term and input values are bound to these placeholders. 
Solution to Find #2:
We replaced the direct concatenation of the username parameter with a placeholder. The input value is now safely bound to the placeholder. 
Solution to Find #3:
